  ...
  <form method="post" action="{{ route('posts.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
    ...
    <div>
      ...
      <textarea id="full_text" name="full_text"
        ...
        :value="old('full_text')" required autocomplete="full_text" rows="5">{{ old('full_text') }}</textarea>
      ...
      <div class="flex flex-wrap gap-2">
        @foreach ($tags as $tag)
          <div class="flex items-center">
            <input {{ is_array(old('tags')) && in_array($tag->id, old('tags')) ? 'checked' : '' }} name="tags[]"
              id="tag-{{ $tag->id }}" type="checkbox" value="{{ $tag->id }}"
